...
use crate::record::{book::ColumnIdentifier, Book, BookError};
...
pub(crate) trait NestedBookView<D: AppDatabase>: BookView<D> {
    fn push_scope(&mut self, search: Search) -> Result<(), BookViewError>;

    fn pop_scope(&mut self) -> bool;
}

pub(crate) struct SearchableBookView<D: AppDatabase> {
